...
void Analyse(int r, double range1, double range2, std::vector<int> triggerchannels, ofstream &ofs, ofstream &ofs2,  int Nevts=-1, string adc="DT5725" )
{
 cout << "run " << r << " Trigger channels "; for(auto t : triggerchannels) cout << t << " "; cout << endl;
  ...
  ana::Run_t myrun(r,{{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/May2021/ROOT/RUN%i_ch2.root",r),"ADC0"},{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/May2021/ROOT/RUN%i_ch4.root",r),"ADC1"},{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/May2021/ROOT/RUN%i_ch5.root",r),"ADC2"}},adc,range1, range2,250,Nevts);
...
void Analyse_April(int r, double range1, double range2, std::vector<int> triggerchannels, ofstream &ofs, ofstream &ofs2,  int Nevts=-1, string adc="DT5725" )
{
 cout << "run " << r << " Trigger channels "; for(auto t : triggerchannels) cout << t << " "; cout << endl;
  ...
  std::vector<double> SPEAmp={38.6,24.8,25.5};//Amplitud del SPE en cuentas de ADC
  for(int i=0;i<3;i++) Gains[i]=Gains[i]/1.602e-7; //(1e-19+12)
  ana::Run_t myrun(r,{{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/Apr2021/ROOT/RUN%i_ch2.root",r),"ADC0"},{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/Apr2021/ROOT/RUN%i_ch4.root",r),"ADC1"},{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/Apr2021/ROOT/RUN%i_ch5.root",r),"ADC2"}},adc,range1, range2,250,Nevts);
  myrun.SetGains(Gains);
  myrun.SetSPEAmps(SPEAmp);
  //~myrun.SelectChannels({0,1,2});
  myrun.SelectChannels({0,1,2});
...
//  myrun.LoopWaveforms(0,"pqr",NULL,1,0);
  myrun.Plot36("Charge_Q3MaxPeakRange",Form("AnalysisROOT_April/Run%i_Q3MaxPeak500ns.root",r));
  myrun.DumpVariableToNtuple(Form("AnalysisROOT_April/Run%i_NTuple.root",r),0);
  myrun.Close();
}
...
{17,{0},10000},//revisar peaktimes
{18,{0},10000},//revisar peaktimes
{19,{0},10000},//revisar peaktimes
{20,{1},10000},//revisar peaktimes
{21,{1},10000},//revisar peaktimes
{22,{1},10000},//revisar peaktimes
{23,{2},10000},//revisar peaktimes
{24,{2},10000},
{25,{2},10000},
{26,{1,2},10000},
{27,{1,2},10000},
...
for(auto r : myvector_april) Analyse_April(r.run, 1.2e-6,1.6e-6,r.triggerChannels,ofs,ofs2, r.Nevts);
