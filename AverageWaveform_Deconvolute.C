...
void Analyse(int r, double range1, double range2, std::vector<int> triggerchannels,int NEvts=-1, string adc="DT5725")
{
  ...
  ana::Run_t myrun(r,{{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/Feb2021/ROOT/RUN%i_ch1.root",r),"ADC0"},{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/Feb2021/ROOT/RUN%i_ch2.root",r),"ADC1"},{Form("/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/Feb2021/ROOT/RUN%i_ch3.root",r),"ADC2"}},adc,range1, range2,30,100);
  myrun.SetGains(Gains);
  myrun.SetSPEAmps(SPEAmp);
//  myrun.SelectChannels(triggerchannels);
  myrun.SelectChannels({1});
  myrun.Process(); 
  myrun.SetCutPedSTD();
...
//  myrun.LoopWaveforms(0,"",NULL,1,-3);

  myrun.SetCutMaxAmplitudeRange(100,2000000);
  if(NEvts!=-1) myrun.SetMaximumWaveformsToProcess(NEvts);
//  myrun.Plot36("ScintProfMyDeconvolution",Form("AnalysisROOT/Run%i_Deconvoluted.root",r),2,0);
  myrun.Plot36("ScintProfDeconvoluteSiPM",Form("AnalysisROOT/Run%i_DeconvolutedTS.root",r),2,1);
  myrun.Close();
}


void AverageWaveform_Deconvolute()
{
...
for (auto r : myvector)
{
  Analyse(r.run, 1.25e-6,1.47e-6,r.triggerChannels, r.Nevts); //2 pmts in coincidence
  //else Analyse(r.run, 1.34e-6,1.47e-6,r.triggerChannels, r.Nevts); //1 single PMT
}
