...
TH1F* Analyze( myrun r)
{
  ...
  ana::Run_t myrun1(r.number,{{r.file,r.channelName}},"DT5725",0.33e-6,0.41e-6,30,-1);
...
void FitChannel(int mych, std::vector<int> runlist, std::vector<std::pair<float,float>> limits, string var="Charge")
{
  ...
  for (auto run : runlist)
  {
    ...
    if(!h[run]) {cout << "histogram " << Form("Run%i_ADC%i_%s",run,mych,var.c_str()) << " NOT FOUND!" << endl;}
     gStyle->SetOptFit(1);
   TF1 *f = new TF1("f","[0]*exp(-0.5*((x-[1])/[2])*((x-[1])/[2]))   +	[3]*exp(-0.5*((x-[4])/[5])*((x-[4])/[5]))+	[6]*exp(-0.5*((x-[7])/[8])*((x-[7])/[8]))",0,16);
    f->SetParameters( 323,3.622,0.8,	50,7,0.7, 	11,13,1);
	f->SetParNames(
	"A_{1}","#mu_{1}","#sigma_{1}",
	"A_{2}","#mu_{2}","#sigma_{2}",
	"A_{3}","#mu_{3}","#sigma_{3}");

    h[run]->Fit("f","QMER+","SAME",1,16);

    //~if(limits[run].first!=limits[run].second){ h[run]->Fit("gaus","QMER+","SAME",limits[run].first,limits[run].second);}
    h[run]->Draw(); gPad->SetLogy();
    lets_pause();
  }
...
//Macro ejecutable de forma escalada, se puede comentar un paso una vez ha terminado, y descomentar el siguiente. El output de un paso es usado como input en el siguiente.
  ...
  folder="20210501"; //directorio donde se volcarán todos los outputs.
  /*
...
// tras ejecutar este paso, puedes echar un vistazo en folder, los pdf generados con los histogramas generados.
  ...
  //Paso 2, hago el fit en el SPE
  std::vector<std::pair<float,float>> limitsch4 = {{2,5}, {3.5,7}, {4.5,9.5} };
  FitChannel(4,{1,2,3},limitsch4,"Charge");
...
//*/
/*
  //Paso 2, hago el fit en el SPE en el histograma de amplitudes.
