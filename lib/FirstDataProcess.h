...
//    cout << endl;
  ...
  bool LoadOscilloscope(TString file, bool DEBUG=false,int Length=52)
  {
    ...
    {
       double time,adc; char c;	  ifs >> time >> c >> adc;      ADCd.push_back(adc);
...
int ReadAndDump(string file, string outfile, bool TS=false)
{
  ...
  if(ifs.is_open())
  {
    ...
    while(!ifs.eof())
    {
      ...
      //if(i==2738){ cout << "Loaded waveform " << i << endl;debug=true;}
      //if(i>68897){ cout << "Loaded waveform " << i << endl; debug=true;}
    }
...
int ReadAndDumpBinary(string file, string outfile)
{
  ...
  while (1)
  {
        ...
        if(wv.size()%20000==0){ cout << "Loaded waveform " << wv.size()<< endl;break;}
        //~if(wv.size()%20000==0){ cout << "Loaded waveform " << wv.size()<< endl;/*break;*/}
...
int ReadAndDumpBinaryLight(string file, string outfile)
{
  ...
  else
  {
   ...
   TFile f(outfile.c_str(),"RECREATE"); f.cd();
   TTree *t = new TTree("IR02","IR02");
   std::vector<short> _u;
   int _EventNumber;
   ULong64_t _TriggerTimeStamp;
  ...
  while (1)
  {
        ...
        if(counter%20000==0){ cout << "Loaded waveform " << counter << endl;}
        //~ if(wv.size()%100000==0){ cout << "Loaded waveform " << wv.size()<< endl;break;}
  }
  fclose(pFile);
