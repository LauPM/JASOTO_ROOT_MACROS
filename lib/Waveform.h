...
//       cout << "SumOfSamples " << SumOfSamples << endl;
	...
	double rangemin3=1e6; int PeakBin=0; int PeakBin_Range=0;
	...
	for(int i=0; i<v->size();i++)
	{
		...
		if (countercut>4){ saturations.push_back(v->at(i));}
                ...
                if(t->at(i)>(PeakTime+range3)&&t->at(i)<(PeakTime+range4))Charge_Range_34+=(v->at(i)-PedestalMean);
                if(t->at(i)>(PeakTime+range5)&&t->at(i)<(PeakTime+range6))Charge_Range_56+=(v->at(i)-PedestalMean);
                for(int k=1;k<Peak_ranges.size();++k){                if(t->at(i)>(PeakTime+Peak_ranges[0])&&t->at(i)<(PeakTime+Peak_ranges[k]))QPeak_ranges[k-1]+=(v->at(i)-PedestalMean); }
                for(int k=1;k<Fixed_ranges.size();++k){                if(t->at(i)>(Fixed_ranges[0])&&t->at(i)<(Fixed_ranges[k]))QFixed_ranges[k-1]+=(v->at(i)-PedestalMean); }
		...
		for(int k=0;k<QFixed_ranges.size();++k){QFixed_ranges[k]=sampling*QFixed_ranges[k]/ConversionFactor;}                        
		for(int k=0;k<QPeak_ranges.size();++k){QPeak_ranges[k]=sampling*QPeak_ranges[k]/ConversionFactor;}                        
