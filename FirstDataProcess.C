...
{//May 2021 Cajita
	...
	//~//NormalRuns
	//~for(int r=27;r<28;r++) for (auto ch:channels) ReadAndDumpBinary(t1+Form("Run%02i/wave%i.dat",r,ch),t2+Form("ROOT/RUN%i_ch%i.root",r,ch)) ;
	 
	 //~//Laser
	//~for (int r=5; r<6; r++) for (int ch=4;ch<6;ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/RUN1002_ch%i.root",ch)) ;
	//~for (int r=12; r<13; r++) for (int ch=2;ch<3;ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/RUN1002_ch%i.root",ch)) ;

	//~//Calibration
	//~for (int r=1; r<5; r++) for (int ch=4; ch<6; ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/Calibration20210527_ch%i_%i.root",ch,r)) ;
	//~for (int r=6; r<12; r++) for (int ch=2; ch<3; ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/Calibration20210527_ch%i_%i.root",ch,r)) ;

	//---------------------28 Mayo-----------------------
	t1="/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/May2021/28Mayo2021/";
	t2="/pnfs/ciemat.es/data/neutrinos/SiPM_dune_IR02/May2021/";

	//NormalRuns
	for(int r=29;r<40;r++) for (auto ch:channels) ReadAndDumpBinary(t1+Form("Run%02i/wave%i.dat",r,ch),t2+Form("ROOT/RUN%i_ch%i.root",r,ch)) ;
	 
	 //~//Laser
	//~for (int r=5; r<6; r++) for (int ch=4;ch<6;ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/RUN1003_ch%i.root",ch)) ;
	//~for (int r=12; r<13; r++) for (int ch=2;ch<3;ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/RUN1003_ch%i.root",ch)) ;

	//~//Calibration
	//~for (int r=1; r<5; r++) for (int ch=4; ch<6; ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/Calibration20210528_ch%i_%i.root",ch,r)) ;
	//~for (int r=6; r<12; r++) for (int ch=2; ch<3; ch++) ReadAndDumpBinary(t1+Form("CAL%02i/wave%i.dat",r,ch),t2+Form("ROOT/Calibration20210528_ch%i_%i.root",ch,r)) ;
